...
export default class WeatherViewComponent {
  ...
  constructor() {
    toObservable(this.city)
      .pipe(
        ...
        switchMap(query =>
          this.weatherService.searchCities(query.trim()).pipe(catchError(() => of([])))
        )
      )
  ...
  selectCity(suggestion: CitySuggestion) {
    ...
    this.selectedCityName.set(`${suggestion.name}, ${suggestion.country}`);
    this.city.set('');
    this.suggestions.set([]);
    ...
    this.loadWeather(suggestion.lat, suggestion.lon);
  }
  ...
  useMyLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        this.loadWeather(latitude, longitude, true);
      },
      () => {
  ...
  onEnter() {
    ...
    this.selectCity(index >= 0 ? cities[index] : cities[0]);
  }

  getDayName(timestamp: number): string {
    return new Date(timestamp * 1000).toLocaleDateString('es-ES', { weekday: 'short' });
  }
  ...
  private loadWeather(lat: number, lon: number, useApiName = false) {
    this.weatherService.getWeatherByCoords(lat, lon).subscribe({
      next: data => {
        if (useApiName) {
          this.selectedCityName.set(`${data.city}, ${data.country}`);
        }
